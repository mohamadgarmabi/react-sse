...
class SSESharedWorker {
  ...
  private handleConnect(event: MessageEvent) {
    ...
    port.onmessage = (incomingMessage: MessageEvent<WorkerMessage>) => {
  ...
  private handleSSEEvent(event: SSEEvent) {
    if (this.currentConnection) {
      ...
      if (this.currentConnection.events.length > 100) {
        this.currentConnection.events = this.currentConnection.events.slice(
          -100
        );
      }
    }
  ...
  private calculateRetryDelay(
    ...
    customRetryFunction?: (attempt: number) => number
  ): number {
